...
module.exports = async (desc) => {
    ...
    async function updateNavbar(){
        ...
        if(isfile){
            ...
            if(file.indexOf('<screenfull') > -1 && file.indexOf('<search ') === -1){
                file = newStr(file,file.indexOf(`<screenfull`),`<HeaderSearch id="header-search" class="right-menu-item" :style="{'color':navbarColor }" />
                        `)
                file = newStr(file,file.indexOf(`export default `),`import HeaderSearch from '@/components/HeaderSearch'
                        `)
                file = newStr(file,file.indexOf(`Hamburger,`),`HeaderSearch,
                        `)
                return Promise.resolve(file)
            }else {
    ...
    async function updateGetters(){
        ...
        if(isfile){
            let  file =  await fscreateReadStream(filePath)
            if(file.indexOf('sidebar: state => state.app.sidebar,') > -1 && file.indexOf('permission_routes:') === -1){
                file = newStr(file,file.indexOf(`sidebar: state => state.app.sidebar,`),`permission_routes: state => state.permission.routes,
                `)
                return Promise.resolve(file)
            }else {
    ...
    try {
        ...
        log('src/store/getters.js文件修改成功')
