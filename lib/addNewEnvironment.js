...
`
    ...
    async function updateapiURL(){
        const filePath = `${pathFile}/src/services/api-url.js`
        const isfile = await isFileExisted(filePath)
        if(isfile){
            let  file =  await fscreateReadStream(filePath)
            ...
            errorLog('找不到api-url.js目标文件，请查看命令是否使用正确')
            return Promise.reject(new Error('Error'))
        }
    }
    ...
    try {
        ...
        await fscreateWriteStream(`${pathFile}/src/services`,`api-url.js`,apiURLFile)
        log('src/services/api-url.js文件修改成功')
