...
module.exports = async (desc) => {
    ...
    async function updateNavbar(rows) {
        ...
        if (isfile) {
            let file = await fscreateReadStream(filePath)
            if (rows.layout === 2) {
                var datastrArr = file.match((/(<div\s*class="logo">([\s\S]*?)<div class="navbarLogo">)/g))
                let str = datastrArr?datastrArr[0]:datastrArr
                ...
                if (file.indexOf(str) > -1) {
                    ...
                    var dataArr = file.match((/(Screenfull,([\s\S]*?)Logo)/g))
                    let Logo = dataArr?dataArr[0]:dataArr
                    file = file.replace(Logo, `Screenfull`)
                ...
                if (file.indexOf(str) > -1) {
                   ...
                   `)
                    var dataArr = file.match((/(Hamburger,([\s\S]*?)Screenfull)/g))
                    let Logo = dataArr?dataArr[0]:dataArr
                    file = file.replace(Logo, `Hamburger,
                        Screenfull,
                        Logo`)
                    return Promise.resolve(file)
                } else {
