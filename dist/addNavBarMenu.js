const{resolve:resolve}=require("path"),{fscreateReadStream:fscreateReadStream,fscreateWriteStream:fscreateWriteStream,isFileExisted:isFileExisted,newStr:newStr}=require("./common.js"),{run:run}=require("runjs"),chalk=require("chalk"),log=e=>console.log(chalk.green(e)),errorLog=e=>console.log(chalk.red(e));module.exports=async t=>{"string"!=typeof t&&(t=""),log("resolve   : "+resolve("./")+"/"+t);let e=resolve("./")+(t&&0!==t.length?`/${t}`:"");try{var r=await async function(){var t=`${e}/src/layout/components/Navbar.vue`;if(await isFileExisted(t)){let e=await fscreateReadStream(t);if(-1<e.indexOf('<div class="right-menu">')&&-1===e.indexOf("<el-menu")){e=newStr(e,e.indexOf('<div class="right-menu">'),`
                  <div style="float: left;height: 100%;line-height: 50px;">
                    <el-menu :default-active="activeIndex" class="el-menu-demo" mode="horizontal" @select="handleSelect">
                      <el-menu-item v-for="(item,i) in addRouters" :key="i" :index="i.toString()">{{ item.meta && item.meta.title }}</el-menu-item>
                        ...
                        `),e=newStr(e,e.indexOf("navbarBackground() {"),`
                    addRouters(){
                      let routers = []
                      ...
                      return routers
                        ...
                        `);t=e.match(/(data\(\)\s*\{([\s\S]*?)return\s*\{)/g),t=t&&t[0];e=-1<e.indexOf(t)?newStr(e,e.indexOf(t)+t.length,`
                  activeIndex: '0',
                    ...
                    `);t="mounted() {";return e=-1<e.indexOf(t)?newStr(e,e.indexOf(t)+t.length,`
                    this.urlhx()
...
`),Promise.resolve(e)}return errorLog(`src/layout/components/Navbar.vue文件修改失败!
                请检查代码:
                ...
                `),Promise.reject(new Error("Error")))}return errorLog("找不到src/store/modules/permission.js目标文件，请查看命令是否使用正确"),Promise.reject(new Error("Error"))}(),a=await async function(){var t=`${e}/src/layout/components/Sidebar/index.vue`;if(await isFileExisted(t)){let e=await fscreateReadStream(t);return-1<e.indexOf("routes() {")&&-1===e.indexOf(":base-path=\"basePath+'/'+route.path\"")?(e=newStr(e,e.indexOf("routes() {"),`
                 basePath() {
                      return this.$store.state.permission.basePath
                    },
                `),e=e.replace("this.$store.state.permission.routes","this.$store.state.permission.typeRoutes"),e=e.replace(':base-path="route.path"',`:base-path="basePath+'/'+route.path"`),Promise.resolve(e)):(errorLog(`src/layout/components/Sidebar/index.vue 文件修改失败!
                请检查代码:
                1、是否有: routes() {
